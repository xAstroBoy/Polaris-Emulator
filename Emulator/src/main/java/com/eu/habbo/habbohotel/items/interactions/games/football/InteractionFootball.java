...
public class InteractionFootball extends InteractionPushable {
    ...
    public int getWalkOnVelocity(RoomUnit roomUnit, Room room) {
        if (roomUnit.getPath().isEmpty()
                && roomUnit.tilesWalked() == 2
                && this.getExtradata().equals("1")) return 0;

        if (roomUnit.getPath().size() == 0 && roomUnit.tilesWalked() == 1) return 6;
    ...
    public int getWalkOffVelocity(RoomUnit roomUnit, Room room) {
        if (roomUnit.getPath().size() == 0 && roomUnit.tilesWalked() == 0) return 6;
    ...
    public int getDragVelocity(RoomUnit roomUnit, Room room) {
        if (roomUnit.getPath().isEmpty() && roomUnit.tilesWalked() == 2) return 0;
    ...
    public RoomUserRotation getWalkOffDirection(RoomUnit roomUnit, Room room) {
        ...
        return RoomUserRotation.values()[
                (RoomUserRotation.values().length
                                + Rotation.Calculate(roomUnit.getX(), roomUnit.getY(), nextWalkTile.x, nextWalkTile.y)
                                + 4)
                        % 8];
    }
            ...
            case NORTH_EAST:
                if (this.validMove(
                        ...
                        room.getLayout()
                                .getTileInFront(
                                        room.getLayout().getTile(this.getX(), this.getY()),
                                        RoomUserRotation.NORTH_WEST.getValue()))) return RoomUserRotation.NORTH_WEST;
                        ...
                        room.getLayout()
                                .getTileInFront(
                                        room.getLayout().getTile(this.getX(), this.getY()),
                                        RoomUserRotation.SOUTH_EAST.getValue()))) return RoomUserRotation.SOUTH_EAST;
                else return RoomUserRotation.SOUTH_WEST;
            ...
            case SOUTH_EAST:
                if (this.validMove(
                        ...
                        room.getLayout()
                                .getTileInFront(
                                        room.getLayout().getTile(this.getX(), this.getY()),
                                        RoomUserRotation.SOUTH_WEST.getValue()))) return RoomUserRotation.SOUTH_WEST;
                else if (this.validMove(
                        room,
                        room.getLayout().getTile(this.getX(), this.getY()),
                        room.getLayout()
                                .getTileInFront(
                                        room.getLayout().getTile(this.getX(), this.getY()),
                                        RoomUserRotation.NORTH_EAST.getValue()))) return RoomUserRotation.NORTH_EAST;
                else return RoomUserRotation.NORTH_WEST;
                ...
                if (this.validMove(
                        ...
                        room.getLayout()
                                .getTileInFront(
                                        room.getLayout().getTile(this.getX(), this.getY()),
                                        RoomUserRotation.NORTH_WEST.getValue()))) return RoomUserRotation.NORTH_WEST;
                else return RoomUserRotation.NORTH_EAST;
                ...
                if (this.validMove(
                        ...
                        room.getLayout()
                                .getTileInFront(
                                        room.getLayout().getTile(this.getX(), this.getY()),
                                        RoomUserRotation.SOUTH_WEST.getValue()))) return RoomUserRotation.SOUTH_WEST;
                else return RoomUserRotation.SOUTH_EAST;
        }
    }
    ...
    public boolean validMove(Room room, RoomTile from, RoomTile to) {
        ...
        BigDecimal topItemHeight =
                BigDecimal.valueOf(topItem.getZ() + topItem.getBaseItem().getHeight());
    ...
    public void onMove(
        ...
        if (game == null) {
            try {
                game = FootballGame.class
                        .getDeclaredConstructor(new Class[] {Room.class})
                        .newInstance(room);
                room.addGame(game);
            } catch (Exception e) {
                return;
        ...
        HabboItem topItem = room.getTopItemAt(to.x, to.y, this);
        if ((topItem != null)
                && ((currentTopItem == null) || (currentTopItem.getId() != topItem.getId()))
                && ((topItem instanceof InteractionFootballGoal))) {
